...
def extract_informations_from_json(data: Dict) -> Tuple[List[Dict], List[Dict], List[Dict]]:
    ...
    _: List[int] = __extract_rule_descriptor_from_json(data['ruleset'], all_rule_descriptor, all_rule_field_descriptor, all_target_value)
...
def __extract_rule_field_descriptor_from_json(rule_field_descriptors: List[Dict], all_rule_field_descriptor: List[Dict], all_target_value: List[Dict]) -> List[int]:
    for ind_rule_f_desc in range(len(rule_field_descriptors)):
        ...
        rule_field_descriptors[ind_rule_f_desc] = __append_item(all_rule_field_descriptor, rule_field_descriptors[ind_rule_f_desc])
    return rule_field_descriptors


def __extract_rule_descriptor_from_json(rule_descriptors: List[Dict], all_rule_descriptor: List[Dict], all_rule_field_descriptor: List[Dict], all_target_value: List[Dict]) -> List[int]:
    for rule_desc in rule_descriptors:
        if 'field_descriptors' in rule_desc:
            rule_desc['field_descriptors'] = __extract_rule_field_descriptor_from_json(rule_desc['field_descriptors'], all_rule_field_descriptor, all_target_value)
